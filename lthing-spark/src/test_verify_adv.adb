--  test_verify_adv — adversarial gate on the public LTHING_MLDSA_Verify_G65.Verify:
...
procedure Test_Verify_Adv is
   ...
   PK    : LTHING_MLDSA_Params_65.Public_Key := V.PK;
   Sg    : LTHING_MLDSA_Params_65.Signature  := V.Sig;
   Fails : Natural := 0;
...
begin
   ...
   Chk ("valid V31 accepts",
        LTHING_MLDSA_Verify_G65.Verify (PK, Msg, Ctx, Sg));

   Sg (100) := Sg (100) xor 1;                --  flip one signature bit
   Chk ("1-bit signature tamper rejects",
        not LTHING_MLDSA_Verify_G65.Verify (PK, Msg, Ctx, Sg));
   ...
   declare
      PK0   : constant LTHING_MLDSA_Params_65.Public_Key := V.PK;   --  untampered
      Empty : constant Byte_Array (1 .. 0) := (others => 0);
   begin
      Chk ("empty message is well-defined and fail-closed",
           not LTHING_MLDSA_Verify_G65.Verify (PK0, Empty, Ctx, V.Sig));
   end;
