--  test_encode87 — round-trip property gate for the ML-DSA-87 encoders.
--
--  The encoders (Pk_Encode/Sig_Encode) are validated against the already-
--  KAT-anchored decoders by the relational identity decode(encode(x)) = x
--  over pseudo-random in-range inputs (per CLAUDE.md: property fact, not a
--  self-derived magic vector).
...
begin
   ---------------------------------------------------------------------------
   --  T1: pkEncode round-trip — rho (32 B) + t1 (k=8 polys, coeffs 0..1023).
   ---------------------------------------------------------------------------
   declare
   ...
   begin
      ...
      for I in T1_Vec'Range loop
         for J in T1 (I)'Range loop
            T1 (I) (J) := LTHING_MLDSA87_Codec.Coeff (Rand_Mod (1024));  --  10-bit
         end loop;
      end loop;
   ...
   ---------------------------------------------------------------------------
   --  T2: sigEncode round-trip — c_tilde (64 B) + z (centered band) + hint.
   ---------------------------------------------------------------------------
   declare
   ...
   begin
      ...
      --  z: centered in -(gamma1-1) .. gamma1, stored canonical in 0..Q-1.
      for I in Z_Vec'Range loop
         for J in Z (I)'Range loop
            declare
               C : constant Integer := Rand_Mod (2 * Gamma1) - (Gamma1 - 1);
            begin
               Z (I) (J) := LTHING_MLDSA87_Codec.Coeff (if C < 0 then C + Q else C);
            end;
         end loop;
      end loop;

      --  hint: <= Omega (75) set bits total; 9 per poly * 8 polys = 72.
      --  Positions strictly increasing per poly.
   ...
   New_Line;
   if Fails = 0 then
      Put_Line ("ENCODE87 GATE PASSED: pk/sig encoders invert the proven decoders");
   else
      Put_Line ("ENCODE87 FAILURES:" & Fails'Image);
