--  test_codec87 — pk/sig decode on an AUTHORITATIVE ML-DSA-87 vector (V63) +
...
procedure Test_Codec87 is
   package M renames MLDSA87_KAT_Vectors;
   V : M.Vector renames M.V63;            --  tcId=63, Expected=True (ACCEPT)
   Rho     : Rho_Array;
   T1      : T1_Vec;
...
begin
   ...
   Chk ("T1 coeffs in 0..1023 (real Level-5 key decode)", T1_OK);

   Sig_Decode (V.Sig, C_Tilde, Z, H, Ok);
   Chk ("Sig_Decode Ok=True on valid ML-DSA-87 vector", Ok);
   ...
   Chk ("hint weight <= omega (75)", HW <= Omega);

   --  Adversarial: corrupt a hint end-pointer (> omega) -> fail-closed (Alg 21).
   --  For ML-DSA-87: Hint_Off = Sig_Bytes - (Omega + K_Dim) = 4627 - 83 = 4544.
   declare
      Bad      : Signature := V.Sig;
      Hint_Off : constant := Sig_Bytes - (Omega + K_Dim);   --  4544
   begin
      Bad (Hint_Off + Omega) := 200;     --  y[omega+0] = 200 > omega(75) -> ⊥
      Sig_Decode (Bad, C_Tilde, Z, H, Ok);
      Chk ("corrupted hint end-pointer -> Ok=False (fail-closed)", not Ok);
   end;

   New_Line;
   if Fails = 0 then Put_Line ("CODEC87 GATE PASSED: pk/sig decode + fail-closed hint");
   else Put_Line ("CODEC87 FAILURES:" & Fails'Image);
