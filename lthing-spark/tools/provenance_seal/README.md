...
- `verify_seal65.adb` — independently verifies an ML-DSA-65 signature
  ...
  fact.
- `rule30_salt.py` — Epoch Rule30 Salt Stream (License §1.12/§5A.3.1):
  ...
  `date_epoch_masked`.
- `build_seal.py` — assembles the canonical LTHING document (4-space indent,
...
gnatmake -q -D obj -aI. -o bin/verify_seal65 verify_seal65.adb
```
...
EPOCH_MS=$(date +%s%3N)
DATE_MASKED=$(python3 rule30_salt.py "$EPOCH_MS" | grep ^date_epoch_masked_hex= | cut -d= -f2)
...
python3 build_seal.py --fields-json seal_fields.json --out PROVENANCE_SEAL.lthing
```

## Verify a seal afterward

Reconstruct the exact signed payload (the canonical document with `seal_id`
present and `signature`/`seal_signer_public_key` absent — see
`canonical_render` in `build_seal.py`), then:
